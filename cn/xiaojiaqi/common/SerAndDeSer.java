...
public class SerAndDeSer {
	...
	public static String[] serializeByCBT(cn.xiaojiaqi.common.TreeNode root) {
		ArrayList<String> level = new ArrayList<>();
		serializeBycbt(root,0,level);
		return level.toArray(new String[level.size()]);
	}
	public static void serializeBycbt(cn.xiaojiaqi.common.TreeNode root, int i, ArrayList<String> level) {
		if(root!=null) {
			TreeNode p = root;
			if(i>=level.size()) {
				int size = level.size();
